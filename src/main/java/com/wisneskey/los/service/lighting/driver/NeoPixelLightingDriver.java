 ...
 */
public class NeoPixelLightingDriver implements LightingDriver {

	private static final Logger LOGGER = LoggerFactory.getLogger(NeoPixelLightingDriver.class);
	...
	/**
	 * The raw-data of all pixels, each int of LEDs is split into bits and
	 * converted to bytes to write
	 */
	 ...
	 */
	public NeoPixelLightingDriver(Context pi4j, int numLEDs, double brightness) {
	 ...
	 */
	public NeoPixelLightingDriver(Context pi4j, int numLEDs, double brightness, int channel) {
		if (numLEDs < 1 || brightness < 0 || brightness > 1 || channel < 0 || channel > 1) {
			throw new IllegalArgumentException("Illegal Constructor");
		}
		
		LOGGER.debug("initialising a LED strip with " + numLEDs + " leds");
	 ...
	 *
	 * @param pixel
	 *          which position on the LED strip, range 0 - numLEDS-1
	 ...
	 * closest to the transmitter. - The most significant bit is always sent
	 * first.
	 * <p>
	...
	/**
	 * Helper Class specific for only LEDStrips and matrices can calculate
	 * different colors, and gets the individual color channels
	 */
	public static class PixelColor {
		 ...
		 *           if {@code hue}, {@code saturation}, {@code brightness} are out
		 *           of range
		 */
